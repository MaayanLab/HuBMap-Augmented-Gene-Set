...
def returnTopGenes(gene_marker_list, organ_coexpression_dict):
    ...
    for organ, coexpression_dict in organ_coexpression_dict:
...
def main():
    ...
    if not myCommandLine.args.p:
        ...
        for file in corr_dir_list:
            ...
            path = "{}/{}/{}".format(cur_dir, myCommandLine.args.c, file)
            coexpression_dict[organ_name] = buildCoexpressionDict(pd.read_csv(path, sep='\t'))
    ...
    print(organs_not_in_coexpression_dict)
    

    # Add top genes to gene set associated with a cell type
    print("Finding top genes...")
    final_dict = dict()
    counts = 0
    for organ, gene_dict in gene_marker_dict.items():
        ...
        for cell_type, gene_set_str in gene_dict.items():
            ...
            final_dict[organ][cell_type] = final_gene_set_list 
            counts += counts
    ...
    for organ, cell_gene_dict in final_dict.items():
        for cell_type, gene_list in cell_gene_dict.items():
            cell_type = re.sub(' (\d*)', '', cell_type)
            fh.write("{}\t".format(cell_type.title()))
